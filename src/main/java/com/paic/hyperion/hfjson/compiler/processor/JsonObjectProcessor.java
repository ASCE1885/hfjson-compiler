...
import com.paic.hyperion.hfjson.annotation.HFJsonIgnore;
import com.paic.hyperion.hfjson.annotation.HFJsonObject;
import com.paic.hyperion.hfjson.annotation.HFJsonObject.FieldDetectionPolicy;
...
public class JsonObjectProcessor extends Processor {
    ...
    public void findAndParseObjects(RoundEnvironment env, Map<String, JsonObjectHolder> jsonObjectMap, Elements elements, Types types) {
        for (Element element : env.getElementsAnnotatedWith(HFJsonObject.class)) {
            try {
    ...
    private void processJsonObjectAnnotation(Element element, Map<String, JsonObjectHolder> jsonObjectMap, Elements elements, Types types) {
        ...
        if (element.getModifiers().contains(PRIVATE)) {
            error(element, "%s: %s annotation can't be used on private classes.", typeElement.getQualifiedName(), HFJsonObject.class.getSimpleName());
        } else if (typeElement.getTypeParameters().size() > 0) {
            error(element, "%s: @%s annotation can't be used on generic classes.", typeElement.getQualifiedName(), HFJsonObject.class.getSimpleName());
        }
        ...
        if (holder == null) {
            ...
            while (superclass.getKind() != TypeKind.NONE) {
                ...
                if (superclassElement.getAnnotation(HFJsonObject.class) != null) {
            ...
            HFJsonObject annotation = element.getAnnotation(HFJsonObject.class);
            ...
            HFJsonObject.FieldDetectionPolicy fieldDetectionPolicy = annotation.fieldDetectionPolicy();
    ...
    private void createOrUpdateFieldHolder(Element element, Elements elements, Types types, JsonObjectHolder objectHolder) {
        if (element.getAnnotation(HFJsonIgnore.class) == null) {
